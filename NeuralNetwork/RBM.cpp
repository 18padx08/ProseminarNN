...
RBM::RBM( int n_v, int n_h, double **w, double *hb, double *vb) : mtx() {
...
void RBM::contrastive_divergence(int *input, double lr, int k, int N) {
...
double RBM::propup(int *v, double *w, double b) {
	...
	for (int j = 0; j<n_visible; j++) {
		pre_sigmoid_activation += w[j] * v[j]/255;
	}
...
double RBM::propdown(int *h, int i, double b) {
	...
	for (int j = 0; j<n_hidden; j++) {
		pre_sigmoid_activation += W[j][i] * h[j]/255;
	}
	pre_sigmoid_activation += b;
	return sigmoid(pre_sigmoid_activation);
}
